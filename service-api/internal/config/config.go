...
// Config is the root application configuration for service-api.
type Config struct {
...
// WorkspaceSizeMB/CleanupDelaySec/AllowedHosts) lived here when service-api
...
// Load reads YAML from disk and applies env overrides (prefix SERVICE_API_).
...
func applyEnvOverrides(cfg *Config) {
	...
	setStr("SERVICE_API_DATABASE_HOST", &cfg.Database.Host)
	setInt("SERVICE_API_DATABASE_PORT", &cfg.Database.Port)
	setStr("SERVICE_API_DATABASE_USER", &cfg.Database.User)
	setStr("SERVICE_API_DATABASE_PASSWORD", &cfg.Database.Password)
	setStr("SERVICE_API_DATABASE_NAME", &cfg.Database.Name)
	setStr("SERVICE_API_READONLY_USER", &cfg.Readonly.User)
	setStr("SERVICE_API_READONLY_PASSWORD", &cfg.Readonly.Password)
	setStr("SERVICE_API_SANDBOX_SERVICE_URL", &cfg.Sandbox.ServiceURL)
	setStr("SERVICE_API_CALLBACK_BASE", &cfg.Sandbox.CallbackBase)
	setInt("SERVICE_API_SERVER_PORT", &cfg.Server.Port)
	// DEEPSEEK_API_KEY is the standard env name; SERVICE_API_AI_REVIEW_API_KEY is
	...
	setStr("SERVICE_API_AI_REVIEW_API_KEY", &cfg.AIReview.APIKey)
	setStr("SERVICE_API_AI_REVIEW_BASE_URL", &cfg.AIReview.BaseURL)
	setStr("SERVICE_API_AI_REVIEW_MODEL", &cfg.AIReview.Model)
	setInt("SERVICE_API_AI_REVIEW_TIMEOUT_SECONDS", &cfg.AIReview.TimeoutSeconds)
	setInt("SERVICE_API_AI_REVIEW_CACHE_TTL_DAYS", &cfg.AIReview.CacheTTLDays)
	if v := os.Getenv("SERVICE_API_AI_REVIEW_ENABLED"); v != "" {

...
- **WHEN** 查看 `service-api/config.yaml` 的 `ai_review` 配置
...
系统 SHALL 使用 `deepseek-reasoner` 模型调用 DeepSeek API，开启 JSON 输出模式。调用超时 SHALL 上限 30 秒。模型与超时通过 `service-api/config.yaml` 的 `ai_review.model` 与 `ai_review.timeout_seconds` 配置，default 为 `deepseek-reasoner` 和 `30`。
...
DeepSeek API key SHALL 由 backend 在 `service-api/config.yaml` 的 `ai_review.api_key` 字段持有。桌面客户端 SHALL NOT 持有、请求、转发该 key。服务端 SHALL NOT 把 key 回显在任何响应、日志、错误信息中。

#### Scenario: 配置文件持有

- **WHEN** 查看 `service-api/config.yaml`

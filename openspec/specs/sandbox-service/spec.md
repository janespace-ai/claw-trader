...
系统 SHALL 新增根目录服务 `sandbox-service/`（与 `service-api/`、`data-aggregator/` 平级），基于 Python 3.11 + FastAPI，以 **单个长驻容器** 的形态运行，负责接收 service-api 推送的执行任务。SHALL NOT 为每个任务新建容器。

#### Scenario: 服务目录结构

- **WHEN** 查看仓库根目录
- **THEN** 存在 `sandbox-service/` 目录，同级于 `service-api/`、`data-aggregator/`、`desktop-client/`
...
sandbox-service SHALL 暴露 `POST /run` 接收来自 service-api 的任务请求，并暴露 `GET /status/{job_id}` 查询状态。这些 endpoint 仅在 `claw-sandbox-net` 内部网络可达，不对外（即不绑定到宿主机端口）。
...
- **WHEN** service-api 调用：
  ...
  Content-Type: application/json
  {
    ...
    "callback_base_url": "http://service-api:8081/internal/cb"
  }
  ```
...
- **WHEN** service-api 调用 `GET /status/<job_id>`
...
- **THEN** 仅从 `claw-sandbox-net` 同网络下的 service-api 可达
...
sandbox-service 的 worker SHALL 通过 HTTP callback 把进度、结果、错误上报给 service-api，保留 `POST /internal/cb/{progress,complete,error}` 现有契约不变。callback base URL 通过每次任务请求由 engine 透传。
...
sandbox-service SHALL 复用当前 `service-api/sandbox/framework/` 下的 Python 代码（`runner.py`、`claw/strategy/`、`claw/screener/` 等），不重写业务逻辑。迁移方式为：把 framework 代码从 service-api 镜像迁移到 sandbox-service 镜像，移除 service-api 对这些文件的构建依赖。

#### Scenario: framework 代码位置

- **WHEN** 查看 sandbox-service 源
- **THEN** 存在 `sandbox-service/src/claw/strategy/` 与 `sandbox-service/src/claw/screener/`，内容来自旧的 `service-api/sandbox/framework/`
- **THEN** service-api 不再保留这些 Python 文件

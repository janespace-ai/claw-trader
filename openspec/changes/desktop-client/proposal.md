...
- 深浅双主题：Dark / Light / Auto（跟系统）三档切换，所有屏幕共用同一份 token 设计
- 多语言：英文 + 简体中文，首次启动检测系统语言，用户可手动切换
- K 线涨跌配色惯例可选：绿涨红跌（Western/TradingView）/ 红涨绿跌（Chinese/红涨绿跌），默认跟语言
- AI 回复语言默认"跟随用户输入"，用户可改为固定英文或中文
- 单用户 MVP，无登录/认证
...
- `theming`: Dark / Light / Auto 三档主题，全局 token 驱动，K 线涨跌色可独立配置
- `i18n`: 英文 + 简体中文 UI 翻译，react-i18next，AI 回复语言跟随用户输入或固定
...
- **前端依赖**: React, Electron, TradingView Lightweight Charts, better-sqlite3, Tailwind CSS, react-i18next, i18next-browser-languagedetector

...
- [x] 1.1 初始化 Electron + React + TypeScript 项目（Vite 构建，electron-builder 打包）
- [x] 1.2 配置项目结构（electron/, src/components, src/services, src/stores, src/hooks）
- [x] 1.3 配置 Tailwind CSS 样式框架
- [x] 1.4 配置 Electron main/renderer 进程通信（IPC preload 脚本）
- [x] 1.5 配置 electron-builder 打包（macOS .dmg + Windows .exe）

## 2. 本地存储层 (SQLite)

- [x] 2.1 集成 better-sqlite3，Electron main process 初始化数据库
- [x] 2.2 创建 strategies 表（id, name, type, code, description, status, is_favorite, tags, version, parent_id, created_at, updated_at）
- [x] 2.3 创建 conversations 表（id, title, messages, strategy_id, created_at, updated_at）
- [x] 2.4 创建 backtest_results 表（id, strategy_id, type, symbols, config, summary_metrics, per_symbol_metrics, equity_curve, trades, remote_task_id, created_at）
- [x] 2.5 创建 coin_lists 表（id, name, symbols, screener_id, created_at, updated_at）
- [x] 2.6 创建 settings 表（key, value）
- [x] 2.7 实现 IPC handlers：strategies CRUD（创建、查询列表、更新状态/收藏、按 ID 查询、按 parent_id 查版本链）
- [x] 2.8 实现 IPC handlers：conversations CRUD（创建、追加消息、查询列表、按 ID 查询）
- [x] 2.9 实现 IPC handlers：backtest_results CRUD（创建、查询列表、按 ID 查询）
- [x] 2.10 实现 IPC handlers：coin_lists CRUD（创建、更新、查询列表）
- [x] 2.11 实现 IPC handlers：settings get/set

## 3. LLM 适配层

- [x] 3.1 定义统一 LLM 接口（ChatMessage, LLMAdapter, stream 方法）
- [x] 3.2 实现 OpenAI Compatible 适配器（覆盖 OpenAI, DeepSeek, Kimi，可配置 baseURL/model）
- [x] 3.3 实现 Anthropic 适配器（Claude Messages API，流式输出）
- [x] 3.4 实现 Google Generative AI 适配器（Gemini，流式输出）
- [x] 3.5 实现 LLM 工厂函数（根据用户设置的 provider 创建对应适配器）
- [x] 3.6 实现 IPC handlers：LLM 流式调用（main process 调 API，通过 IPC 逐块推送到 renderer）

## 4. Prompt 管理与上下文构造

- [x] 4.1 编写策略生成 system prompt（Strategy 基类规范、可用指标、交易方法、参数优化格式）
- [x] 4.2 编写选币生成 system prompt（Screener 基类规范、数据限制 1h/4h/1d、元数据字段）
- [x] 4.3 编写策略优化 system prompt（分析回测结果、识别问题、生成改进代码）
- [x] 4.4 实现上下文构造器（context-builder）：回测结果智能摘要（核心指标 + 亏损最大 N 笔 + 连续亏损 + 多空对比）
- [x] 4.5 实现上下文裁剪：旧消息压缩为摘要，控制 token 预算在模型上限 70% 内
- [x] 4.6 实现意图识别辅助：根据当前 Tab 和对话内容注入对应的 system prompt

## 5. 远程 API 客户端

- [x] 5.1 实现 backtest-engine API 客户端（baseURL 可配置，默认 http://localhost:8081）
- [x] 5.2 实现回测任务提交（POST /api/backtest/start）
- [x] 5.3 实现回测进度轮询（GET /api/backtest/status/:task_id，3 秒间隔）
- [x] 5.4 实现回测结果获取（GET /api/backtest/result/:task_id）
- [x] 5.5 实现选币任务提交（POST /api/screener/start）
- [x] 5.6 实现选币结果获取（GET /api/screener/result/:task_id）
- [x] 5.7 实现策略代码提交（POST /api/strategies）
- [x] 5.8 实现连接状态检测（健康检查 + 断线重连指示）

## 6. 状态管理 (Zustand)

- [x] 6.1 实现 conversationStore（当前对话、消息列表、AI 流式输出状态）
- [x] 6.2 实现 strategyStore（当前策略、策略列表、版本链）
- [x] 6.3 实现 coinListStore（当前选币列表、手动增删操作）
- [x] 6.4 实现 backtestStore（回测状态、进度、当前结果、历史结果）
- [x] 6.5 实现 settingsStore（API Key、默认模型、远程服务地址）
- [x] 6.6 实现 appStore（回测就绪状态计算：策略 ✓ + 选币 ✓）

## 7. 基础 UI 组件

- [x] 7.1 实现 App 主布局（左侧主内容 + 右侧 AI 面板，面板宽度可拖拽，可折叠）
- [x] 7.2 实现顶部导航栏（Tab: 选币/策略/回测，设置入口）
- [x] 7.3 实现设置页面（API Key 配置、模型选择、远程服务地址、连接状态）
...
- [x] 10.1 实现策略列表页（名称、类型、状态、收藏、最近回测摘要、时间）
- [x] 10.2 实现策略筛选（全部/收藏/有效/无效 + 关键词搜索）
- [x] 10.3 实现策略操作（查看详情、发起回测、复制、标记有效/无效、收藏/取消收藏）
- [x] 10.4 实现策略代码预览（Python 语法高亮，只读模式）
- [x] 10.5 实现策略版本历史（展示版本链，可回退查看旧版本）

## 11. 回测结果展示

- [x] 11.1 实现回测准备状态指示器（策略 ✅/⬜ + 选币 ✅/⬜ + 开始按钮）
- [x] 11.2 实现预回测结果页（K线图 + 信号标注 + 简要指标 + 交易列表 + 确认/修改按钮）

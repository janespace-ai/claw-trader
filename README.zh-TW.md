...
- **每個交易思路一個工作區。** 每條「策略」就是一段聊天——打開就看到完整決策記錄、當前程式碼、當前幣種列表、最近一次回測結果。下次回來從你離開的地方繼續。
- **跟 AI 對話,拿到策略程式碼。** 用自然語言描述想法,AI 策略師幫你寫程式碼。每次改動都是一張 diff 預覽卡,你點套用或拒絕。
- **AI 也幫你篩幣。** 說一句「篩 24h 成交額 top 30」,AI 真的去後端跑篩選,把通過的幣寫進當前工作區。
- **觀察它在歷史上本來會怎樣。** 當程式碼和幣列表都齊了,工作區自動跑一次回測。聚合收益 + 按幣種下鑽,看「這套策略在主流幣上獲利,在小幣上虧損」。
- **聊天裡直接調參。** 輸入「試 RSI 14, 21, 28」,工作區派發參數掃描,挑出最佳組合寫回策略。
- **滿意才儲存。** 只有點 [保存策略] 才把當前 draft 定格;聊天驅動的改動一直累積在草稿區。
...
- **指標**:純 numpy / pandas 實作(`claw.indicators`)——保持沙箱映像檔精簡

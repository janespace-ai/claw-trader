...
echo "---- bringing up service-api ----"
docker compose -f "$BE_DIR/docker-compose.yml" up -d --build

# Poll /healthz on 8081 until service-api answers.
echo "---- waiting for service-api /healthz ----"
...
if ! curl -fsS http://localhost:8081/healthz >/dev/null 2>&1; then
  echo "service-api did not become healthy" >&2
  docker logs --tail 60 claw-service-api
  exit 1
fi
